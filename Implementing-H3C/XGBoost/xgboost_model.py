!pip install -q python-docx xgboost openpyxl
...
def run_models_on_df(df_raw, dataset_name, default_rate=0.10, default_dur=36, default_lgd=0.5, test_size=0.3, random_state=42):
    ...
    n1 = (y_train == 1).sum()
    n0 = (y_train == 0).sum()
    scale_pos_weight = 1.0 if n0 == 0 or n1 == 0 else (n1 / n0 if n1 >= n0 else 1.0 / (n1 / n0))
    ...
    # Modèles (seulement XGBoost)
    common_params = dict(max_depth=3, n_estimators=200, random_state=random_state, eval_metric='logloss')
    models = {
        "XGBoost CLASSIC": XGBClassifier(**common_params),
        "XGBoost CS": XGBClassifier(**common_params, scale_pos_weight=scale_pos_weight),
        "XGBoost H3C": XGBClassifier(**common_params, scale_pos_weight=scale_pos_weight)
    }
    ...
    for name, model in models.items():
        # Entraînement
        if name in ["XGBoost CS", "XGBoost H3C"]:
            model.fit(X_train, y_train, sample_weight=mci_train if name == "XGBoost CS" else h3c_train)
        else:
    ...
    results_df = pd.DataFrame({
        ...
        'duration': dur_te,
        'y_true': y_test,
        'y_pred_classic': predictions["XGBoost CLASSIC"]["y_pred"],
        'cost_mci_classic': predictions["XGBoost CLASSIC"]["cost_mci"],
        'y_pred_cs': predictions["XGBoost CS"]["y_pred"],
        'cost_mci_cs': predictions["XGBoost CS"]["cost_mci"],
        'y_pred_h3c': predictions["XGBoost H3C"]["y_pred"],
        'cost_h3c': predictions["XGBoost H3C"]["cost_h3c"]
    })
...
# 3) Rapport Word
doc = Document()
doc.add_heading("Rapport — Évaluation XGBoost Credit Scoring", 0)

doc.add_heading("1. Introduction", level=1)
doc.add_paragraph(
    "Ce rapport évalue trois modèles XGBoost (Classic, CS, H3C) sur différents datasets de credit scoring. "
...
doc.add_paragraph(
    ...
    "XGBoost H3C et CS optimisent les coûts, mais l'accuracy peut varier. "
...
word_path = "rapport_credit_scoring_xgboost.docx"
doc.save(word_path)
