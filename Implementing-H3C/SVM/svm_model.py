!pip install python-docx sklearn openpyxl
...
# 📌 Mapping des colonnes métier par dataset
column_mapping = {
...
# Define lgd_mapping
lgd_mapping = {key: value.get("lgd", 0.5) for key, value in column_mapping.items()}
...
def run_models_on_df(df_raw, dataset_name, default_rate=0.10, default_dur=36, default_lgd=0.5, test_size=0.3, random_state=42):
    df = df_raw.copy()
    mapping = column_mapping.get(dataset_name, {})
    lgd = lgd_mapping.get(dataset_name, default_lgd)
    ...
    n1 = (y_train == 1).sum()
    n0 = (y_train == 0).sum()
    class_weight = 'balanced' if n0 > 0 and n1 > 0 else None
    ...
    mci_train = np.where(mci_train <= 0, 1.0, mci_train)  # Assurer des poids positifs
    mci_train = mci_train / (mci_train.mean() + 1e-12)

    h3c_tmp = compute_h3c(y_train, 1 - y_train, amt_tr, rate_tr, dur_tr, lgd=lgd)
    h3c_train = np.where(h3c_tmp <= 0, 1.0, h3c_tmp)  # Assurer des poids positifs
    h3c_train = h3c_train / (h3c_train.mean() + 1e-12)

    # Modèles (SVM)
    common_params = dict(kernel='rbf', probability=True, random_state=random_state)
    models = {
        "SVM CLASSIC": SVC(**common_params),
        "SVM CS": SVC(**common_params, class_weight=class_weight),
        "SVM H3C": SVC(**common_params, class_weight=class_weight)
    }
    ...
    for name, model in models.items():
        # Entraînement
        if name in ["SVM CS", "SVM H3C"]:
            model.fit(X_train, y_train, sample_weight=mci_train if name == "SVM CS" else h3c_train)
        else:
    ...
    # Construire le DataFrame des résultats détaillés (colonnes restreintes)
    results_df = pd.DataFrame({
        'amount': amt_te,
        'rate': rate_te,
        'duration': dur_te,
        'y_true': y_test,
        'y_pred_classic': predictions["SVM CLASSIC"]["y_pred"],
        'cost_mci_classic': predictions["SVM CLASSIC"]["cost_mci"],
        'y_pred_cs': predictions["SVM CS"]["y_pred"],
        'cost_mci_cs': predictions["SVM CS"]["cost_mci"],
        'y_pred_h3c': predictions["SVM H3C"]["y_pred"],
        'cost_h3c': predictions["SVM H3C"]["cost_h3c"]
    })
...
folder_path = '/content/drive/My Drive/files_CS'
dataset_files = list(column_mapping.keys())
...
for fname in dataset_files:
    ...
    if not os.path.exists(file_path):
        print(f"⚠️ Fichier {fname} introuvable dans {folder_path}")
        continue
    df = None
    sep = dataset_separators.get(fname, ",")
    for enc in encodings:
        try:
            df = pd.read_csv(file_path, encoding=enc, sep=sep, engine='python', quoting=csv.QUOTE_MINIMAL)
            if df.shape[1] == 1:
                continue
        ...
        for enc in encodings:
            for alt_sep in seps:
                if alt_sep == sep:
                    continue
                try:
                    df = pd.read_csv(file_path, encoding=enc, sep=alt_sep, engine='python', quoting=csv.QUOTE_MINIMAL)
                    if df.shape[1] == 1:
                        continue
...
# 💾 Exports
# 1) Excel : une feuille par dataset avec colonnes restreintes
excel_path = "resultats_par_jeu.xlsx"
...
csv_path = "resume_global.csv"
summary_df.to_csv(csv_path, index=False, sep=';')
print(f"📄 CSV global écrit: {csv_path}")

# 3) Rapport Word
doc = Document()
doc.add_heading("Rapport — Évaluation SVM Credit Scoring", 0)

doc.add_heading("1. Introduction", level=1)
doc.add_paragraph(
    "Ce rapport évalue trois modèles SVM (Classic, CS, H3C) sur différents datasets de credit scoring. "
...
doc.add_paragraph(
    ...
    "H3C : 0.7×coût_mci + 0.2×(0.01×montant) + 0.1×(γ×montant). Modèles entraînés avec stratification si possible."
)
...
doc.add_paragraph(
    ...
    "Les modèles SVM CS et H3C utilisent des poids pour optimiser les coûts, mais l'accuracy peut varier. "
    "Les datasets avec cibles manquantes (ex. german_credit_data.csv) ou valeurs invalides peuvent être ignorés."
)

doc.add_heading("6. Conclusion", level=1)
doc.add_paragraph(
    "Cette pipeline généralisée traite plusieurs datasets avec harmonisation des colonnes. "
    "Optimisations futures : réglage des hyperparamètres (C, gamma), gestion robuste des valeurs manquantes dans target, validation des données d'entrée."
)

word_path = "rapport_credit_scoring_svm.docx"
doc.save(word_path)
