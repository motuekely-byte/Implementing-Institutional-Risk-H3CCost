!pip install -q python-docx sklearn openpyxl
...
def run_models_on_df(df_raw, dataset_name, default_rate=0.10, default_dur=36, default_lgd=0.5, test_size=0.3, random_state=42):
    ...
    # Modèles (Régression Logistique)
    common_params = dict(max_iter=1000, random_state=random_state)
    models = {
        "Logistic CLASSIC": LogisticRegression(**common_params),
        "Logistic CS": LogisticRegression(**common_params, class_weight=class_weight),
        "Logistic H3C": LogisticRegression(**common_params, class_weight=class_weight)
    }
    ...
    for name, model in models.items():
        # Entraînement
        if name in ["Logistic CS", "Logistic H3C"]:
            model.fit(X_train, y_train, sample_weight=mci_train if name == "Logistic CS" else h3c_train)
        else:
    ...
    results_df = pd.DataFrame({
        ...
        'duration': dur_te,
        'y_true': y_test,
        'y_pred_classic': predictions["Logistic CLASSIC"]["y_pred"],
        'cost_mci_classic': predictions["Logistic CLASSIC"]["cost_mci"],
        'y_pred_cs': predictions["Logistic CS"]["y_pred"],
        'cost_mci_cs': predictions["Logistic CS"]["cost_mci"],
        'y_pred_h3c': predictions["Logistic H3C"]["y_pred"],
        'cost_h3c': predictions["Logistic H3C"]["cost_h3c"]
    })

    return results_df, summary_rows

# 📥 Chargement des données (upload manuel)
print("Sélectionne tes fichiers CSV…")
uploaded = files.upload()  # Permet de sélectionner plusieurs fichiers CSV
...
# 3) Rapport Word
doc = Document()
doc.add_heading("Rapport — Évaluation Régression Logistique Credit Scoring", 0)

doc.add_heading("1. Introduction", level=1)
doc.add_paragraph(
    "Ce rapport évalue trois modèles de régression logistique (Classic, CS, H3C) sur différents datasets de credit scoring. "
...
doc.add_paragraph(
    ...
    "Les modèles Logistic CS et H3C utilisent des poids pour optimiser les coûts, mais l'accuracy peut varier. "
...
doc.add_paragraph(
    ...
    "Optimisations futures : réglage des hyperparamètres, gestion robuste des valeurs manquantes dans target, validation des données d'entrée."
)

word_path = "rapport_credit_scoring_logreg.docx"
doc.save(word_path)
