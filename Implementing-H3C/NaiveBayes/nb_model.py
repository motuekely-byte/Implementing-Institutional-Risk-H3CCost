...
def run_models_on_df(df_raw, dataset_name, default_rate=0.10, default_dur=36, default_lgd=0.5, test_size=0.3, random_state=42):
    ...
    # Modèles (Naive Bayes)
    common_params = dict()  # GaussianNB n'a pas beaucoup de paramètres
    models = {
        "Naive Bayes CLASSIC": GaussianNB(**common_params),
        "Naive Bayes CS": GaussianNB(**common_params),
        "Naive Bayes H3C": GaussianNB(**common_params)
    }
    ...
    for name, model in models.items():
        # Entraînement
        if name in ["Naive Bayes CS", "Naive Bayes H3C"]:
            model.fit(X_train, y_train, sample_weight=mci_train if name == "Naive Bayes CS" else h3c_train)
        else:
    ...
    results_df = pd.DataFrame({
        ...
        'duration': dur_te,
        'y_true': y_test,
        'y_pred_classic': predictions["Naive Bayes CLASSIC"]["y_pred"],
        'cost_mci_classic': predictions["Naive Bayes CLASSIC"]["cost_mci"],
        'y_pred_cs': predictions["Naive Bayes CS"]["y_pred"],
        'cost_mci_cs': predictions["Naive Bayes CS"]["cost_mci"],
        'y_pred_h3c': predictions["Naive Bayes H3C"]["y_pred"],
        'cost_h3c': predictions["Naive Bayes H3C"]["cost_h3c"]
    })
...
excel_path = "resultats_par_jeu_naive_bayes.xlsx"
...
csv_path = "resume_global_naive_bayes.csv"
summary_df.to_csv(csv_path, index=False, sep=';')
print(f"📄 CSV global écrit: {csv_path}")

# 3) Rapport Word
doc = Document()
doc.add_heading("Rapport — Évaluation Naive Bayes Credit Scoring", 0)

doc.add_heading("1. Introduction", level=1)
doc.add_paragraph(
    "Ce rapport évalue trois modèles Naive Bayes (Classic, CS, H3C) sur différents datasets de credit scoring. "
...
doc.add_paragraph(
    ...
    "Naive Bayes H3C et CS optimisent les coûts, mais l'accuracy peut varier. "
...
word_path = "rapport_credit_scoring_naive_bayes.docx"
doc.save(word_path)
